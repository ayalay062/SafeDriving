...
export class ViewActiveOffersComponent implements OnInit {
  ...
  filter() {
    ...
    if (this.dateFilter) {
      this.offers = this.offers.filter(x => new Date(x.date_time).getMonth() === this.dateFilter.getMonth()
        && new Date(x.date_time).getFullYear() === this.dateFilter.getFullYear()
        && new Date(x.date_time).getDate() === this.dateFilter.getDate()
